...
export class AShitService {
  ...
  getShitsByToiletId(toiletId: number, jointables: boolean): Observable<AShit[]> {
    return this.http.get<AShit[]>(
      `${this.baseUrl}/ashit?toiletId=${toiletId}&jointables=${jointables}`,
    );
  }
}
